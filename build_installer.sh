...
build_installer() {
	/usr/local/bin/platypus -AR  -l -i 'kidsruby.icns' -a "KidsRuby $VERSION Installer for OSX" -o 'Progress Bar' -p '/bin/sh' -u 'The Hybrid Group + Friends'  -V "$1"  -I 'org.kidsruby.installer' -f 'resources/git-1.7.6-i386-snow-leopard.dmg' -f 'resources/ruby-1.9.2-p320.universal.tar.gz' -f 'resources/yaml-0.1.4.universal.tar.gz' -f 'resources/qt-mac-opensource-4.7.3.dmg' -f 'resources/qtbindings-4.7.3-universal-darwin-10.gem' -f 'resources/kidsruby.tar.gz' -f 'resources/kidsirb.sh' -f 'resources/KidsRuby.app.tar.gz' -f "resources/gosu-0.7.36.2-universal-darwin.gem" -f "resources/htmlentities-4.3.0.gem" -f "resources/rubywarrior-i18n-0.0.3.gem" -f "resources/serialport-1.1.1-universal.x86_64-darwin-10.gem" -f "resources/hybridgroup-sphero-1.0.1.gem" -f "resources/install_gems.sh" -f "resources/i18n/en.sh" -g '#000000'  -b '#ffffff'  -c 'installer.sh' 'build/KidsRubyInstaller.app'
}
